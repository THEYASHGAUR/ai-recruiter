              ...
              <div className="mb-5 p-5 bg-blue-50 rounded-xl border border-blue-100">
                <div className="flex items-start justify-between">
                  ...
                  <div className="ml-4">
                    <button
                      onClick={async () => {
                        const textToCopy = buildJdTextForCopy(
                          jdSummaryParsed ?? results?.jd_summary
                        );
                        try {
                          await navigator.clipboard.writeText(textToCopy);
                          showToast(
                            "JD summary copied to clipboard",
                            "success"
                          );
                        } catch {
                        ...
                        {jd.requirements && (
                          <div>
                            ...
                            <div className="text-sm text-slate-700 space-y-1">
                              ...
                              {Array.isArray(jd.requirements.skills) &&
                                jd.requirements.skills.length > 0 && (
                                  <div className="mt-2 flex flex-wrap gap-2">
                                    {jd.requirements.skills.map(
                                      (s: string, i: number) => (
                                        <span
                                          key={i}
                                          className="px-2 py-1 bg-white border border-slate-200 rounded-full text-xs text-slate-700"
                                        >
                                          {s}
                                        </span>
                                      )
